...
function renderClient() {
  return render(<AudienceProvider><SegurosClient /></AudienceProvider>)
}
...
describe('SegurosClient', () => {
  ...
  it('renders the H1 about proteção', () => {
    renderClient()
    expect(screen.getByRole('heading', { level: 1, name: /Proteção que acompanha/i })).toBeInTheDocument()
  })

  it('renders all 4 benefit categories', () => {
    renderClient()
    ;['Vida', 'Auto', 'Residencial', 'Empresarial'].forEach((t) => {
  ...
  it('renders the lead form labelled "Seguros"', () => {
    renderClient()
